...
TEST(IGraph, Creating) {
  ...
  IGraph<int> *matGr = new MatrixGraph<int>(4);
...
TEST(ListGraph, Simple) {
  std::shared_ptr<IGraph<int>> listGr = std::dynamic_pointer_cast<IGraph<int>>(
      std::make_shared<ListGraph<int>>());
...
TEST(MatrixGraph, Cycled) {
  std::shared_ptr<IGraph<int>> matGr = std::dynamic_pointer_cast<IGraph<int>>(
      std::make_shared<MatrixGraph<int>>(3001));
...
TEST(ArcGraph, Cycled) {
  std::shared_ptr<IGraph<int>> arcGr =
      std::dynamic_pointer_cast<IGraph<int>>(std::make_shared<ArcGraph<int>>());
...
// 1 --> 2 --> 3 --> 1
TEST(IGraph, Copying) {
  IGraph<int> *matGr = new MatrixGraph<int>(3001);
  matGr->AddEdge(1, 2, 10);
  matGr->AddEdge(2, 3000, 20);
  matGr->AddEdge(3000, 1, 30);
