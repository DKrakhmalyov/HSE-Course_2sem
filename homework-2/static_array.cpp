...
template<typename T, size_t sz>
static_array<T, sz>::static_array(size_t copacity)
  : _copacity(copacity)
  , _current_size(0) {
...
template<typename T, size_t sz>
size_t static_array<T, sz>::current_size() const {
  return this->_current_size;
}

template<typename T, size_t sz>
size_t static_array<T, sz>::size() const {
...
template<typename T, size_t sz>
static_array<T, sz>::iterator static_array<T, sz>::emplace(size_t index, T &&obj) {
  if (index >= _copacity) {
...
template<typename T, size_t sz>
template<class... Args>
static_array<T, sz>::iterator static_array<T, sz>::emplace(size_t index, Args &&... args) {
  if (index >= _copacity) {
    throw std::out_of_range("static_array: Index out of range");
  }
  if (_initialized[index]) {
...
template<typename T, size_t sz>
void static_array<T, sz>::erase(static_array::iterator iter) {
...
template<typename T, size_t sz>
T& static_array<T, sz>::at(size_t index) {
  if (index >= _copacity) {
    throw std::out_of_range("static_array: Index out of range");
  }
  if (!_initialized[index]) {
...
template<typename T, size_t sz>
T* static_array<T, sz>::_get_pointer_on_index(size_t index) const {
  return reinterpret_cast<T*>(this->_data) + index;
}


template<typename T, size_t sz>
static_array<T, sz>::iterator::iterator(size_t index, static_array* owner)
  : _index(index)
  , _owner(owner)
{ };

template<typename T, size_t sz>
static_array<T, sz>::iterator& static_array<T, sz>::iterator::operator=(const iterator& other) {
  _index = other._index;
  _owner = other._owner;
  return *this;
}
...
template<typename T, size_t sz>
bool static_array<T, sz>::iterator::operator==(const iterator& other) const {
